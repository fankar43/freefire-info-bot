...
class InfoCommands(commands.Cog):
    ...
    f"📝 **└─ Signature**: {social_info.get('signature', 'None') or 'None'}"
            ...
            embed.add_field(name="", value="\n".join([
    "📊 **┌ ACCOUNT ACTIVITY**",
    f"🕹️ **├─ Most Recent OB**: {basic_info.get('releaseVersion', '?')}",
    f"🎟️ **├─ Current BP Badges**: {basic_info.get('badgeCnt', 'Not found')}",
    f"🏆 **├─ BR Rank**: {'' if basic_info.get('showBrRank') else 'Not found'} {basic_info.get('rankingPoints', '?')}",
    f"⚔️ **├─ CS Rank**: {'' if basic_info.get('showCsRank') else 'Not found'} {basic_info.get('csRankingPoints', '?')}",
    ...
    f"⏰ **└─ Last Login**: {self.convert_unix_timestamp(int(basic_info.get('lastLoginAt', 'Not found')))}"
            ...
            embed.add_field(name="", value="\n".join([
    "🧩 **┌ ACCOUNT OVERVIEW**",
    f"🖼️ **├─ Avatar ID**: {profile_info.get('avatarId', 'Not found')}",
    ...
    f"🎚️ **└─ Pet Level**: {pet_info.get('level', 'Not Found')}"
]), inline=False)
        ...
        f"👥 **├─ Live Members**: {clan_info.get('memberNum', 'Not found')}/{clan_info.get('capacity', '?')}"
            ...
            f"   🏷️ **├─ Title**: {captain_info.get('title', 'Not found')}",
            f"   🎟️ **├─ BP Badges**: {captain_info.get('badgeCnt', '?')}",
